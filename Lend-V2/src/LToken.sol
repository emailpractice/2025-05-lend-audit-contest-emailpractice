...
abstract contract LToken is LTokenInterface, ExponentialNoError, TokenErrorReporter {
    ...
    function initialize(
        ...
        require(err == NO_ERROR, "setting lendtroller failed");
    ...
    function transferTokens(address spender, address src, address dst, uint256 tokens) internal returns (uint256) {
        ...
        // unused function
        // lendtroller.transferVerify(address(this), src, dst, tokens);
    ...
    function mintFresh(address minter, uint256 mintAmount) internal {
        ...
        // unused function
        // lendtroller.mintVerify(address(this), minter, actualMintAmount, mintTokens);
    }
    ...
    function borrowFresh(address payable borrower, uint256 borrowAmount) internal {
         ...
         *  Note: The lToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the lToken borrowAmount less of cash.
         *  doTransferOut reverts if anything goes wrong, since we can't be sure if side effects occurred.
         */
    ...
    function repayBorrowFresh(address payer, address borrower, uint256 repayAmount) internal returns (uint256) {
         ...
         *  Note: The lToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the lToken holds an additional repayAmount of cash.
    ...
    function liquidateBorrowFresh(
        ...
        /* Verify lTokenCollateral market's block number equals current block number */
        if (lTokenCollateral.accrualBlockNumber() != getBlockNumber()) {
    ...
    /**
     * @notice Transfers collateral tokens (this market) to the liquidator.
     * @dev Called only during an in-kind liquidation, or by liquidateBorrow during the liquidation of another lToken.
     *  Its absolutely critical to use msg.sender as the seizer lToken and not a parameter.
     * @param seizerToken The contract seizing the collateral (i.e. borrowed lToken)
     * @param liquidator The account receiving seized collateral
     * @param borrower The account having collateral seized
     * @param seizeTokens The number of lTokens to seize
     */
    ...
    function _setLendtroller(LendtrollerInterface newLendtroller) public override returns (uint256) {
        ...
        require(newLendtroller.isLendtroller(), "marker method returned false");
    ...
    function _addReservesFresh(uint256 addAmount) internal returns (uint256, uint256) {
         ...
         *  Note: The lToken must handle variations between ERC-20 and ETH underlying.
         *  On success, the lToken holds an additional addAmount of cash.
         *  doTransferIn reverts if anything goes wrong, since we can't be sure if side effects occurred.
         *  it returns the amount actually transferred, in case of a fee.
         */
